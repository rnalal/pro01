...
<%
	String pid = "";
	if(session.getAttribute("boid")!=null){
		pid = (String) session.getAttribute("boid");
	}
            ...
            <section class="page" id="page1">
                ...
                <div class="tb_wrap">
					<table class="table" id="tb1">
						<thead>
							<tr><th>연번</th><th>제목</th><th>내용</th><th>작성자</th><th>작성일</th><th>직권삭제</th></tr>
						</thead>
						<tbody>
</body>
<%
		...
		try {
			...
			try {
				...
				sql = "select * from board order by bodate desc";
				try {
...
<%							
						...
						while(rs.next()){
							++i;
							kid = rs.getString("boid");
%>
							<tr>
								<td><a href='<%=path %>/young/boardDetail.jsp?id=<%=kid %>'><%=i %></a></td>
								...
								<td>
									<% if(!kid.equals("young")) { %>
									<a href='<%=path %>/young/board_del.jsp?id=<%=kid %>' class="btn btn-primary">삭제</a>
									<% } %>
								</td>
							</tr>
<%							
...
<%
							if(kid.equals("young")){
%>						
								<a href="<%=path %>/young/boardinsert.jsp" class="btn btn-primary">게시글 직권 등록</a>
<%
							} else {
%>
